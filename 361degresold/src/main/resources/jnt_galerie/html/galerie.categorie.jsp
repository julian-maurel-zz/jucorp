...
<c:set var="refList" value="select * from [jnt:fileReference] as ref where ISDESCENDANTNODE(ref,'${renderContext.mainResource.node.path}') order by ref.[j:nodename] desc"/>
<jcr:sql var="listQuery" sql="${refList}"/>

  <div style="spacer:both;"/>
<div id="content_display" style="visibility: visible;">	
  <c:forEach items="${listQuery.nodes}" var="picReference">
    <div style="float:left;"><template:module node="${picReference}" view="renderGallery" /></div>
  </c:forEach>
</div>
