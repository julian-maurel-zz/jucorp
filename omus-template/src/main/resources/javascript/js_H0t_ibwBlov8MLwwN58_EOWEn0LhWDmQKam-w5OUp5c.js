...
(function ($) {
  $(document).ready(function() {
    if ($('body').hasClass('page-node') && $('body').hasClass('node-type-member-state')) {
      $( ".accordion" ).accordion({
        collapsible: true,
        icons: false,
        header: 'h3',
        ...
        create: function(event, ui) {
          $(ui.header).children('span.fa-plus').removeClass('fa-plus').addClass('fa-minus');
        },
        beforeActivate: function( event, ui ) {
          $(ui.oldHeader).children('span.fa-minus').removeClass('fa-minus').addClass('fa-plus');
          $(ui.newHeader).children('span.fa-plus').removeClass('fa-plus').addClass('fa-minus');
        }
      });
    ...
    if (document.documentElement.clientWidth > 991) {
	   $('div.social-media').stickyfloat( {duration: 0, stickToBottom: false, offsetY: 145, cssTransition: true} ); 
	}

  	// Strip out link from News menu item
  	//$('li.sf-item-5 a').attr('href', 'javascript:void(0)');
  	
  	// hides search box for mobile as soon as the DOM is ready
  	/*
  	if (document.documentElement.clientWidth < 768) {
	  	$('.search-form input.textbox').hide();
		
		$('.search-form input.textbox.show').show("fast");
		
		// toggle search box on click
		$('h4.toggle a').click(function() {
		$(this).addClass('hidden');
		$('.search-form input.textbox').addClass('show');
		
		
		//$('#navbar').click(function()  {
		//$('.search-form input.textbox').removeClass('show');
		//$('h4.toggle a').removeClass('hidden');
		//});
		...
		//$('.search-form input.textbox').removeClass('show');
		//$('h4.toggle a').removeClass('hidden');  
		// *hide the field if empty* and resurface the original button, or do a toggle function that swaps out the class...
		//});
  	...
  	// Add Bootstrap menu attributes to mobile nav
  	$('ul.language-switcher-locale-url').addClass('nav navbar-nav navbar-right');
  	$('ul.language-switcher-locale-url').attr('role', 'menu');
