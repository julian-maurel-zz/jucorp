...
<c:if test="${empty currentResource.moduleParams.base}">
    <c:if test="${empty baseline or baseline.string eq 'home'}">
        <c:set var="current" value="${renderContext.mainResource.node}"/>
...
<c:if test="${not empty renderContext.mainResource.node}">
    <jcr:node var="current" path="${renderContext.mainResource.node.resolveSite.home.path}" />
</c:if>
                ...
                <c:if test="${(startLevelValue < navMenuLevel or inpath) and correctType}">
                    <div class="omnu-menu-item">
                        ...
                        <c:if test="${hasChildren}">
                            <template:include view="omnu_principal">
